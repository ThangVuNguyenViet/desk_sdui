part of 'package:desk_sdui_demo/screens/themed_counter.dart';

ScreenBinding get themed_counterBinding => ScreenBinding(
  name: 'themed_counter',
  ir: IrTree(
    name: 'themed_counter',
    version: 1,
    root: WidgetNode(
      name: 'Container',
      args: {
        'color': MemberAccessNode(
          target: MemberAccessNode(
            target: MethodCallNode(
              receiver: null,
              name: 'Theme.of',
              args: [
                RefNode(['context']),
              ],
            ),
            name: 'colorScheme',
          ),
          name: 'primaryContainer',
        ),
        'child': WidgetNode(
          name: 'Center',
          args: {
            'child': WidgetNode(
              name: 'Column',
              args: {
                'mainAxisAlignment': RefNode(['MainAxisAlignment', 'center']),
                'children': ListNode([
                  WidgetNode(
                    name: 'Text',
                    args: {
                      'data': StringInterpNode([
                        RefNode(['vm', 'value']),
                      ]),
                      'style': MemberAccessNode(
                        target: MemberAccessNode(
                          target: MethodCallNode(
                            receiver: null,
                            name: 'Theme.of',
                            args: [
                              RefNode(['context']),
                            ],
                          ),
                          name: 'textTheme',
                        ),
                        name: 'headlineLarge',
                      ),
                    },
                  ),
                  WidgetNode(
                    name: 'Row',
                    args: {
                      'mainAxisAlignment': RefNode([
                        'MainAxisAlignment',
                        'center',
                      ]),
                      'children': ListNode([
                        WidgetNode(
                          name: 'ElevatedButton',
                          args: {
                            'onPressed': EventNode(['vm', 'decrement']),
                            'child': WidgetNode(
                              name: 'Text',
                              args: {'data': ConstNode('-')},
                            ),
                          },
                        ),
                        WidgetNode(
                          name: 'SizedBox',
                          args: {'width': ConstNode(16)},
                        ),
                        WidgetNode(
                          name: 'ElevatedButton',
                          args: {
                            'onPressed': EventNode(['vm', 'increment']),
                            'child': WidgetNode(
                              name: 'Text',
                              args: {'data': ConstNode('+')},
                            ),
                          },
                        ),
                      ]),
                    },
                  ),
                ]),
              },
            ),
          },
        ),
      },
    ),
  ),
  inputs: [
    InputBinding(name: 'context', read: (v) => v as dynamic),
    InputBinding(name: 'vm', read: (v) => v as dynamic),
  ],
  methodRefs: {
    'vm': ['decrement', 'increment'],
  },
  reactives: const [],
);
